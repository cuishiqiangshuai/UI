...
-(void)requestHomelist{
    //GET请求方式
    //合并url
    NSURL *url=[NSURL URLWithString:[NSString stringWithFormat:@"%@?access_token%@",[BASEURL stringByAppendingPathComponent:GETACCESSTOKENPATH],[AccessToken shareHandel].access_token]];
    //请求数据
    NSURLSession *session=[NSURLSession sharedSession];
    NSURLSessionDataTask *task=[session dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        //判断是否请求成功
        
        //请求到的数据（data）转换成字典，也就是JSON解析
